...
class TranscribeCallRecording implements ShouldQueue
{
     ...
     */
    /**
     * Recompute the speaker map for an already-transcribed call from its
     * stored segments — no AssemblyAI re-transcription. Used to backfill
     * speaker names after agent-resolution improvements (e.g. incoming-call
     * agents resolved via forwarded_to).
     */
    public function reidentifySpeakers(CallTranscript $transcript): array
    {
        ...
        if ($speakerMap !== []) {
            $transcript->update(['speaker_map' => $speakerMap]);
        }
