...
PROMPT;
        ...
        // Clamp to column sizes — the LLM occasionally writes past VARCHAR
        // limits (caller_intent 255, sentiment 16) and MySQL rejects the row.
        $callerIntent = $parsed['caller_intent'] ?? null;
        if (is_string($callerIntent)) {
            $callerIntent = \Illuminate\Support\Str::limit(trim($callerIntent), 252);
        }
        $sentiment = $parsed['sentiment'] ?? null;
        if (is_string($sentiment)) {
            $sentiment = mb_substr(trim($sentiment), 0, 16);
        }
