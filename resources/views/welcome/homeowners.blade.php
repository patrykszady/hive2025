...
<x-guest-layout>
        ...
        <div class="px-6 mx-auto max-w-7xl lg:px-8 space-y-24">
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center">
                <div class="lg:order-last">
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{{ __('Your schedule, always up to date') }}</h2>
                    <p class="mt-4 text-lg leading-8 text-gray-600 dark:text-gray-300">
                        {{ __('Know when the crew is coming and what they will be working on. When the plan changes—weather, a late delivery, an added task—you get a text, so you are never caught off guard.') }}
                    </p>
                    <ul class="mt-8 space-y-4">
                        @foreach ([
                            __('Upcoming visits and milestones you can see anytime'),
                            __('A live schedule link sent right to your phone'),
                            __('Automatic texts when dates move'),
                            __('Reminders before scheduled work'),
                        ] as $item)
                            <li class="flex gap-3 text-base text-gray-700 dark:text-gray-300">
                                <flux:icon name="check-circle" class="w-6 h-6 shrink-0 text-indigo-600 dark:text-indigo-400" />
                                <span>{{ $item }}</span>
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center">
                <div>
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{{ __('Talk to your contractor in one place') }}</h2>
                    <p class="mt-4 text-lg leading-8 text-gray-600 dark:text-gray-300">
                        {{ __('Ask questions, share photos, and get answers without digging through old text threads. Every message about your project lives together, so nothing important gets lost.') }}
                    </p>
                    <ul class="mt-8 space-y-4">
                        @foreach ([
                            __('One clean conversation tied to your project'),
                            __('Send and receive photos from the job'),
                            __('A history you can scroll back through anytime'),
                            __('Replies in your language if you prefer'),
                        ] as $item)
                            <li class="flex gap-3 text-base text-gray-700 dark:text-gray-300">
                                <flux:icon name="check-circle" class="w-6 h-6 shrink-0 text-indigo-600 dark:text-indigo-400" />
                                <span>{{ $item }}</span>
                ...
                <div class="rounded-2xl bg-indigo-600 p-8 sm:p-10 shadow-xl">
                    <div class="space-y-4">
                        <div class="max-w-[85%] rounded-2xl bg-white/95 px-4 py-3 text-gray-900">
                            <p class="text-xs font-semibold text-gray-500">{{ __('Your contractor') }}</p>
                            <p class="mt-1 text-sm leading-relaxed">{{ __('Electrical rough-in is done—here are a couple photos before drywall 👍') }}</p>
                        </div>
                        <div class="ml-auto max-w-[85%] rounded-2xl bg-indigo-500/60 px-4 py-3 text-white">
                            <p class="text-xs font-semibold text-indigo-100">{{ __('You') }}</p>
                            <p class="mt-1 text-sm leading-relaxed">{{ __('Looks great! Are we still on for inspection Thursday?') }}</p>
                        </div>
                        <div class="max-w-[85%] rounded-2xl bg-white/95 px-4 py-3 text-gray-900">
                            <p class="text-xs font-semibold text-gray-500">{{ __('Your contractor') }}</p>
                            <p class="mt-1 text-sm leading-relaxed">{!! __('Yes—9 AM Thursday. I&rsquo;ll text you when the inspector confirms ✅') !!}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center">
                <div class="lg:order-last">
                    <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{{ __('Review, approve, and pay—from your phone') }}</h2>
                    <p class="mt-4 text-lg leading-8 text-gray-600 dark:text-gray-300">
                        {{ __('Estimates, change orders, and invoices arrive as clean documents you can read and sign on any device. See exactly what you approved, what you have paid, and where your allowances stand.') }}
                    </p>
                    <ul class="mt-8 space-y-4">
                        @foreach ([
                            __('Review and e-sign estimates and change orders'),
                            __('See invoices and a running total of what is paid'),
                            __('Track selections and allowances in one place'),
                            __('No printing, no app to download'),
                        ] as $item)
                            <li class="flex gap-3 text-base text-gray-700 dark:text-gray-300">
                                <flux:icon name="check-circle" class="w-6 h-6 shrink-0 text-indigo-600 dark:text-indigo-400" />
                                <span>{{ $item }}</span>
                ...
                <div class="p-6 rounded-2xl bg-gray-50 dark:bg-zinc-900 ring-1 ring-gray-200 dark:ring-zinc-800">
                    <div class="p-5 rounded-xl bg-white dark:bg-zinc-950 ring-1 ring-gray-200 dark:ring-zinc-800">
                        <div class="flex items-center justify-between">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('Change order · Tile upgrade') }}</p>
                            <span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-700 dark:bg-amber-400/10 dark:text-amber-300">{{ __('Needs your OK') }}</span>
                        </div>
                        <div class="mt-4 space-y-2 text-sm">
                            <div class="flex justify-between text-gray-700 dark:text-gray-300"><span>{{ __('Upgraded porcelain tile') }}</span><span>{{ __('+$1,400') }}</span></div>
                            <div class="flex justify-between text-gray-700 dark:text-gray-300"><span>{{ __('Allowance remaining') }}</span><span>{{ __('$600') }}</span></div>
                            <div class="flex justify-between pt-2 mt-2 font-semibold text-gray-900 border-t border-gray-200 dark:border-zinc-800 dark:text-white"><span>{{ __('Your cost') }}</span><span>{{ __('$800') }}</span></div>
                        </div>
                        <button type="button" class="w-full px-3 py-2 mt-4 text-sm font-semibold text-white bg-indigo-600 rounded-md">{!! __('Review &amp; sign') !!}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <x-marketing.portal-links
        :heading="__('Everything in your homeowner portal')"
        :subheading="__('One simple, private place for your entire project—tap any area to see how it works.')"
    />
    ...
    <div class="py-20 bg-white dark:bg-zinc-950 sm:py-28">
        <div class="px-6 mx-auto max-w-7xl lg:px-8">
            <div class="max-w-2xl mx-auto text-center">
                <h2 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-4xl">{{ __('Getting in takes about 30 seconds') }}</h2>
                <p class="mt-4 text-lg leading-8 text-gray-600 dark:text-gray-300">
                    {{ __('There is nothing to buy and nothing to install. If your contractor uses Hive, you are a tap away.') }}
                </p>
            </div>
            <div class="grid max-w-2xl grid-cols-1 mx-auto mt-16 gap-8 lg:max-w-none lg:grid-cols-3">
                @php
                    $steps = [
                        ['1', 'envelope', __('Your contractor invites you'), __('You get a text or email with a secure link to your project—no account setup required.')],
                        ['2', 'finger-print', __('Open your private link'), __('Tap to sign in safely. No app to download and no password to remember.')],
                        ['3', 'home-modern', __('See everything about your project'), __('Status, schedule, messages, documents, and payments—all in one simple place.')],
                    ];
                @endphp
            ...
            <div class="grid max-w-3xl grid-cols-1 mx-auto mt-16 gap-6 sm:grid-cols-3">
                @php
                    $assurances = [
                        ['gift', __('Free for homeowners'), __('You never pay to view your project.')],
                        ['lock-closed', __('Private & secure'), __('Only you and your contractor can see it.')],
                        ['device-phone-mobile', __('No app to download'), __('Works right in your phone&rsquo;s browser.')],
                    ];
                @endphp
