<div>
    ...
    {{-- <form wire:submit="add">
        <div class="flex gap-2">
            <input wire:model="draft" type="text" class="grow rounded-full shadow-sm shadow-slate-300 px-5 py-3" placeholder="New Task">
        </div>
    </form> --}}
    {{-- <flux:button
    ...
    <div x-sort="$wire.sort($key, $position)" x-sort:group="tasks" class="grid gap-3">
        ...
        @foreach($this->tasks as $task)
            <div
                ...
                {{-- <button wire:click="remove({{ $task->id }})" type="button" class="transition opacity-0 group-hover:[body:not(.sorting)_&]:opacity-100 text-slate-500 hover:bg-emerald-100/75 hover:text-emerald-700 rounded-full p-2">
    ...
    {{-- @foreach ($this->tasks as $task)
        <div class="px-3 py-1 flex gap-2 items-center">
            <div class="transition translate-x-[-1.5rem] group-hover:[body:not(.sorting)_&]:translate-x-0 text-sm text-slate-600">{{ $task->title }}</div>
        </div>
    @endforeach --}}
