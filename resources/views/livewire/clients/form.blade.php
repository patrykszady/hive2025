    ...
    <form wire:submit="{{$view_text['form_submit']}}" class="grid gap-6">
        ...
        <div
            ...
            <flux:select wire:model="address_selection" variant="combobox">
                ...
                @foreach ($address_suggestions as $address)
                    <flux:select.option value="{{$address['place_id']}}" :key="$address['place_id']">{{$address['description']}}</flux:select.option>
                @endforeach
            </flux:select>

            <div x-show="$wire.form.address">
                <flux:fieldset>
