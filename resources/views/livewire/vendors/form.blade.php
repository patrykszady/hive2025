<x-form-modal name="vendors_form_modal" :title="$view_text['card_title']">
    <div x-data="{ 
        ...
        @if(!$isRegistration && isset($this->form->vendor) && $this->form->vendor->id)
        <div x-show="activeTab === 'expenses'" class="space-y-4">
            <livewire:categories.vendor-category-card :vendor="$this->form->vendor" :embedded="true" :key="'cat-card-' . $this->form->vendor->id" />
        </div>
        @endif
    </div>
