...
// Disable automatic loading unless explicitly set
if ($loading === null) {
    $loading = false;
}
        ...
        <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
            ...
            <?php if ($loading || $countOfTrailingIcons > 0): ?>
                <div class="absolute top-0 bottom-0 flex items-center gap-x-1.5 pe-2 border-e border-transparent end-0 text-xs text-zinc-400">
                    ...
                    <?php if ($clearable): ?>
                        <flux:input.clearable :$clearable :$iconVariant :$iconClasses />
                    <?php endif; ?>

                    <?php if ($copyable): ?>
                        <flux:input.copyable :$copyable :$iconVariant :$iconClasses />
                    <?php endif; ?>

                    <?php if ($viewable): ?>
                        <flux:input.viewable :$viewable :$iconVariant :$iconClasses />
                    <?php endif; ?>

                    <?php if ($expandable): ?>
                        <flux:input.expandable :$expandable :$iconVariant :$iconClasses />
                    <?php endif; ?>
